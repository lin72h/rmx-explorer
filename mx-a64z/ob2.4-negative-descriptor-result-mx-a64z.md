# mx-a64z OB2.4 Negative Descriptor Result
...
`make run AGENT=mx-a64z` completed all probes but exited nonzero because
...
macos-validation/results/mx-a64z/20260513-26.5-25.5.0/m2_invalid_descriptor_disposition.json
macos-validation/results/mx-a64z/20260513-26.5-25.5.0/m2_dead_name_descriptor_right.json
macos-validation/results/mx-a64z/20260513-26.5-25.5.0/m2_double_move_send_descriptor.json
```
...
Validated: 24 files, 24 pass, 0 fail
```
...
OB2.4 should not be closed from the `mx-a64z` run alone. The invalid
disposition and double `MOVE_SEND` probes are clean on native Apple Silicon,
but `dead_name_descriptor_right` found that a dead-name source is delivered and
consumed rather than rejected. The raw JSON is committed so `mx-x64z` can run
the same implementation and the parent can decide whether this is accepted
macOS contract behavior or a gate failure.

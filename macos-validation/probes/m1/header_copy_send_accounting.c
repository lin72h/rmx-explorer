/*
 * header_copy_send_accounting.c - Header COPY_SEND accounting oracle probe.
 *
 * Test ID: macos_m1_header_copy_send_accounting
 *
 * Sends one same-process Mach message whose header remote-port disposition is
 * MACH_MSG_TYPE_COPY_SEND. The probe records source send urefs before send,
 * immediately after mach_msg(SEND), and after mach_msg(RECEIVE). The hard
 * stop/pass condition for OB1.4 is that COPY_SEND must not change source send
 * urefs at SEND return.
 */
...
main(void)
{
    ...
    if (kr_send_refs_before == KERN_SUCCESS &&
        ...
        send_msg.header.msgh_id = OB14_MSG_ID;
    ...
    bool send_refs_before_exact = (kr_send_refs_before == KERN_SUCCESS &&
        send_refs_before == 1);
    bool send_refs_unchanged_at_send =
        (kr_send_refs_after_send == KERN_SUCCESS &&
        send_refs_after_send == send_refs_before);
    ...
    bool type_after_send_exact = (kr_type_after_send == KERN_SUCCESS &&
        type_after_send == MACH_PORT_TYPE_SEND_RECEIVE);
    bool type_after_receive_exact = (kr_type_after_receive == KERN_SUCCESS &&
        type_after_receive == MACH_PORT_TYPE_SEND_RECEIVE);
    bool received_id_matches = (mr_receive == MACH_MSG_SUCCESS &&
        received_msgh_id == OB14_MSG_ID);
    ...
    if (nx_baseline_blocks_probe(&before)) {
        ...
        notes = "send refs before COPY_SEND were not exactly one";
    } else if (!type_before_exact) {
        status = NX_STATUS_FAIL;
        sclass = NX_CLASS_EXACT_CONTRACT;
        notes = "port type before COPY_SEND was not SEND_RECEIVE";
    } else if (mr_send != MACH_MSG_SUCCESS) {
        status = NX_STATUS_PROBE_FAILURE;
        sclass = NX_CLASS_PROBE_FAILURE;
        notes = "mach_msg SEND failed";
    } else if (!send_refs_unchanged_at_send) {
        status = NX_STATUS_FAIL;
        sclass = NX_CLASS_EXACT_CONTRACT;
        notes = "COPY_SEND changed sender send urefs at SEND return";
    } else if (!type_after_send_exact) {
        status = NX_STATUS_FAIL;
        sclass = NX_CLASS_EXACT_CONTRACT;
        notes = "port type after SEND was not SEND_RECEIVE";
    ...
    } else if (kr_send_refs_after_receive != KERN_SUCCESS) {
        status = NX_STATUS_PROBE_FAILURE;
        sclass = NX_CLASS_PROBE_FAILURE;
        notes = "mach_port_get_refs after RECEIVE failed";
    } else if (!type_after_receive_exact) {
        status = NX_STATUS_FAIL;
        sclass = NX_CLASS_EXACT_CONTRACT;
        notes = "port type after RECEIVE was not SEND_RECEIVE";
    ...
    nx_result_emit_header(&j,
        agent,
        "macos_m1_header_copy_send_accounting",
        NULL,
        NULL,
        status,
        sclass);
    ...
    nx_result_emit_return(&j, "mach_msg_send_copy_send",
    ...
    nx_result_emit_right_delta(&j,
        "header COPY_SEND at SEND return",
        "service_port",
        "MACH_PORT_RIGHT_SEND",
        kr_send_refs_before == KERN_SUCCESS ? send_refs_before : -1,
        kr_send_refs_after_send == KERN_SUCCESS ? send_refs_after_send : -1,
        -1,
        -1,
        "unchanged");
    nx_result_emit_right_delta(&j,
        "header COPY_SEND after RECEIVE",
        "service_port",
        "MACH_PORT_RIGHT_SEND",
        kr_send_refs_after_send == KERN_SUCCESS ? send_refs_after_send : -1,
    ...
    nx_json_key_int(&j, "send_urefs_after_send",
        kr_send_refs_after_send == KERN_SUCCESS ? send_refs_after_send : -1);
    nx_json_key_bool(&j, "send_urefs_unchanged_at_send_return",
        send_refs_unchanged_at_send);

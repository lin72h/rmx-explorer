 ...
 *   - task_info(TASK_BASIC_INFO) succeeds with the expected in/out count
...
main(void)
{
    ...
    basic_count = TASK_BASIC_INFO_COUNT;
    kr_basic = task_info(task_self_a, TASK_BASIC_INFO,
                         (task_info_t)&basic_info, &basic_count);

    mach_msg_type_number_t bad_count = TASK_BASIC_INFO_COUNT;
    kr_bad_flavor = task_info(task_self_a, (task_flavor_t)0xffffff,
                              (task_info_t)&basic_info, &bad_count);

    mach_msg_type_number_t null_count = TASK_BASIC_INFO_COUNT;
    kr_null_task = task_info(MACH_PORT_NULL, TASK_BASIC_INFO,
    ...
    if (task_self_a == MACH_PORT_NULL) {
        ...
        status = NX_STATUS_PROBE_FAILURE; sclass = NX_CLASS_PROBE_FAILURE;
        notes = "task_info(TASK_BASIC_INFO) failed";
    ...
    nx_result_emit_return(&j, "task_info_basic",

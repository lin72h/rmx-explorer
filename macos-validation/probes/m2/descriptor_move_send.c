/*
 * descriptor_move_send.c - Descriptor MOVE_SEND oracle probe.
 *
 ...
 * message containing a port descriptor with MACH_MSG_TYPE_MOVE_SEND. The
...
#define OB22_DESCRIPTOR_MSG_ID 0x4f423232
#define OB22_VERIFY_MSG_ID     0x4f423233
#define OB22_STATUS_VERSION    1
#define OB22_TIMEOUT_SECONDS   15
...
} ob22_descriptor_recv_msg_t;

typedef struct {
    mach_msg_header_t header;
} ob22_simple_send_msg_t;

typedef struct {
    mach_msg_header_t header;
    mach_msg_max_trailer_t trailer;
} ob22_simple_recv_msg_t;
#endif
...
static bool
send_refs_are_zero_or_absent(kern_return_t kr, mach_port_urefs_t refs)
{
...
static bool
port_type_has_send(mach_port_type_t type)
{
...
{
    alarm(OB22_TIMEOUT_SECONDS);

    ob22_child_status_t st;
    memset(&st, 0, sizeof(st));
    st.version = OB22_STATUS_VERSION;
    ...
    if (st.kr_child_type_before_send == KERN_SUCCESS &&
        ...
        ob22_descriptor_send_msg_t msg;
        ...
        msg.header.msgh_id = OB22_DESCRIPTOR_MSG_ID;
    ...
    if (st.mr_child_send_descriptor == MACH_MSG_SUCCESS) {
        ...
        ob22_simple_recv_msg_t verify_msg;
...
main(void)
{
    ...
    ob22_child_status_t child;
    memset(&child, 0, sizeof(child));
    child.version = OB22_STATUS_VERSION;
    ...
    if (child_pid > 0) {
        ob22_descriptor_recv_msg_t recv_msg;
        ...
        if (mr_parent_receive_descriptor == MACH_MSG_SUCCESS) {
            ...
            if (kr_parent_delivered_refs == KERN_SUCCESS &&
                ...
                ob22_simple_send_msg_t verify_msg;
                memset(&verify_msg, 0, sizeof(verify_msg));
                ...
                verify_msg.header.msgh_id = OB22_VERIFY_MSG_ID;
    ...
    bool child_send_refs_before_exact =
        (child.kr_child_refs_before_send == KERN_SUCCESS &&
        child.child_refs_before_send == 1);
    bool child_send_refs_consumed_after_send =
        (child_send_refs_before_exact &&
        send_refs_are_zero_or_absent(child.kr_child_refs_after_send,
            child.child_refs_after_send));
    bool child_send_refs_consumed_after_verify =
        (child_send_refs_before_exact &&
        send_refs_are_zero_or_absent(child.kr_child_refs_after_verify,
            child.child_refs_after_verify));
    bool child_type_lost_send_after_send =
        ...
        !port_type_has_send(child.child_type_after_send));
    bool child_type_lost_send_after_verify =
        (child.kr_child_type_before_send == KERN_SUCCESS &&
        child.kr_child_type_after_verify == KERN_SUCCESS &&
        !port_type_has_send(child.child_type_after_verify));
    ...
    bool delivered_right_usable =
        ...
        child.child_verify_received_msgh_id == OB22_VERIFY_MSG_ID);
    ...
    } else if (!child_send_refs_before_exact) {
        status = NX_STATUS_FAIL;
        sclass = NX_CLASS_EXACT_CONTRACT;
        notes = "child cargo send refs before MOVE_SEND were not exactly one";
    } else if (!child_type_lost_send_after_send) {
        status = NX_STATUS_FAIL;
        sclass = NX_CLASS_EXACT_CONTRACT;
        notes = "descriptor MOVE_SEND did not remove the child cargo send type";
    } else if (!child_send_refs_consumed_after_send) {
        status = NX_STATUS_FAIL;
        sclass = NX_CLASS_EXACT_CONTRACT;
        notes = "descriptor MOVE_SEND did not consume child cargo send urefs";
    ...
    } else if (!child_type_lost_send_after_verify) {
        status = NX_STATUS_FAIL;
        sclass = NX_CLASS_EXACT_CONTRACT;
        notes = "child cargo type regained a send right after verification";
    } else if (!child_send_refs_consumed_after_verify) {
        status = NX_STATUS_FAIL;
        sclass = NX_CLASS_EXACT_CONTRACT;
        notes = "child cargo send refs were not consumed after verification";
    ...
    nx_result_emit_return(&j, "child_mach_msg_send_descriptor_move_send",
    ...
    nx_result_emit_right_delta(&j,
        "descriptor MOVE_SEND child sender",
        "cargo_port",
        ...
        "consumed");
    nx_result_emit_right_delta(&j,
        "descriptor MOVE_SEND parent delivered",
        ...
        "usable send right");
    nx_result_emit_right_delta(&j,
        "descriptor MOVE_SEND child after verification",
        "cargo_port",
        "MACH_PORT_RIGHT_SEND",
        child.kr_child_refs_after_send == KERN_SUCCESS ?
            (long long)child.child_refs_after_send : -1,
    ...
    nx_json_key_bool(&j, "child_cargo_type_lost_send_after_send",
        child_type_lost_send_after_send);
    ...
    nx_json_key_int(&j, "child_cargo_send_urefs_after_send",
        child.kr_child_refs_after_send == KERN_SUCCESS ?
            (long long)child.child_refs_after_send : -1);
    nx_json_key_bool(&j, "child_cargo_send_urefs_consumed_after_send",
        child_send_refs_consumed_after_send);
    ...
    nx_json_key_bool(&j, "child_cargo_type_lost_send_after_verify",
        child_type_lost_send_after_verify);
    nx_json_key_bool(&j, "child_cargo_send_urefs_consumed_after_verify",
        child_send_refs_consumed_after_verify);
    ...
    nx_json_key_int(&j, "parent_delivered_deallocate_count",
        delivered_port != MACH_PORT_NULL ? 1 : 0);
    nx_json_key_bool(&j, "parent_delivered_one_deallocate_sufficient",
        kr_parent_deallocate_delivered == KERN_SUCCESS && cleanup_ok);

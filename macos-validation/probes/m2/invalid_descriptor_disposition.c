/*
 * invalid_descriptor_disposition.c - Invalid port descriptor disposition probe.
 *
...
#define OB24_INVALID_MSG_ID      0x4f423241
#define OB24_INVALID_DISPOSITION 0xffu
...
main(void)
{
    ...
    kern_return_t kr_service_refs_before = KERN_FAILURE;
    kern_return_t kr_service_refs_after = KERN_FAILURE;
    kern_return_t kr_cargo_refs_before = KERN_FAILURE;
    kern_return_t kr_cargo_refs_after = KERN_FAILURE;
    kern_return_t kr_cargo_type_before = KERN_FAILURE;
    kern_return_t kr_cargo_type_after = KERN_FAILURE;
    kern_return_t kr_deallocate_delivered = KERN_FAILURE;
    ...
    mach_port_urefs_t service_refs_before = 0;
    mach_port_urefs_t service_refs_after = 0;
    mach_port_urefs_t cargo_refs_before = 0;
    mach_port_urefs_t cargo_refs_after = 0;
    mach_port_type_t cargo_type_before = 0;
    mach_port_type_t cargo_type_after = 0;
    unsigned int sent_msgh_bits = 0;
    unsigned int received_msgh_bits = 0;
    unsigned int received_descriptor_count = 0;
    unsigned int received_descriptor_type = 0;
    unsigned int received_descriptor_disposition = 0;
    ...
    if (kr_insert_cargo_send == KERN_SUCCESS) {
        kr_service_refs_before = mach_port_get_refs(mach_task_self(),
            service_port, MACH_PORT_RIGHT_SEND, &service_refs_before);
    ...
    if (kr_service_refs_before == KERN_SUCCESS &&
        ...
        msg.header.msgh_id = OB24_INVALID_MSG_ID;
        msg.body.msgh_descriptor_count = 1;
        msg.cargo.name = cargo_port;
        msg.cargo.disposition = OB24_INVALID_DISPOSITION;
    ...
    kr_service_refs_after = mach_port_get_refs(mach_task_self(),
        service_port, MACH_PORT_RIGHT_SEND, &service_refs_after);
    kr_cargo_refs_after = mach_port_get_refs(mach_task_self(),
        cargo_port, MACH_PORT_RIGHT_SEND, &cargo_refs_after);
    kr_cargo_type_after = mach_port_type(mach_task_self(), cargo_port,
        &cargo_type_after);

    if (kr_alloc_service == KERN_SUCCESS) {
        ob24_invalid_recv_msg_t recv_msg;
        ...
        if (mr_receive == MACH_MSG_SUCCESS) {
            ...
            received_descriptor_type = recv_msg.cargo.type;
            received_descriptor_disposition = recv_msg.cargo.disposition;
            if (delivered_port != MACH_PORT_NULL) {
                kr_deallocate_delivered = mach_port_deallocate(
    ...
    bool send_rejected = (mr_send != MACH_MSG_SUCCESS);
    bool no_delivery = (mr_receive == MACH_RCV_TIMED_OUT);
    bool service_refs_unchanged =
        (kr_service_refs_before == KERN_SUCCESS &&
        kr_service_refs_after == KERN_SUCCESS &&
        service_refs_before == service_refs_after);
    bool cargo_refs_unchanged =
        (kr_cargo_refs_before == KERN_SUCCESS &&
        kr_cargo_refs_after == KERN_SUCCESS &&
        cargo_refs_before == cargo_refs_after);
    bool cargo_type_unchanged =
        (kr_cargo_type_before == KERN_SUCCESS &&
        kr_cargo_type_after == KERN_SUCCESS &&
        cargo_type_before == cargo_type_after);
    ...
    if (nx_baseline_blocks_probe(&before)) {
        ...
        sclass = NX_CLASS_PROBE_FAILURE;
        notes = "setup failed";
    } else if (!send_rejected) {
        status = NX_STATUS_FAIL;
        notes = "invalid descriptor disposition was not rejected";
    } else if (!no_delivery) {
        status = NX_STATUS_FAIL;
        notes = "invalid descriptor disposition delivered a message";
    } else if (!service_refs_unchanged || !cargo_refs_unchanged ||
        !cargo_type_unchanged) {
        status = NX_STATUS_FAIL;
        notes = "invalid descriptor disposition mutated sender rights";
    ...
    nx_json_key_string(&j, "disposition", hex32(OB24_INVALID_DISPOSITION));
    nx_json_key_string(&j, "right_type_before", "MACH_PORT_TYPE_SEND");
    nx_json_key_string(&j, "right_type_after", nx_port_type_str(cargo_type_after));
    nx_json_end_object(&j);
    nx_json_end_array(&j);
    nx_json_end_object(&j);
    ...
    nx_result_emit_return(&j, "mach_port_insert_right_cargo_make_send",
        ...
        false, 0);
    nx_result_emit_return(&j, "mach_port_get_refs_service_before_send",
        nx_kern_return_str(kr_service_refs_before), kr_service_refs_before,
        false, 0);
    ...
    nx_result_emit_return(&j, "mach_port_type_cargo_before_send",
        ...
        false, 0);
    nx_result_emit_return(&j, "mach_msg_send_invalid_descriptor_disposition",
        nx_msg_return_str(mr_send), mr_send, false, 0);
    nx_result_emit_return(&j, "mach_port_get_refs_service_after_send",
        nx_kern_return_str(kr_service_refs_after), kr_service_refs_after,
        false, 0);
    nx_result_emit_return(&j, "mach_port_get_refs_cargo_after_send",
        nx_kern_return_str(kr_cargo_refs_after), kr_cargo_refs_after,
        false, 0);
    nx_result_emit_return(&j, "mach_port_type_cargo_after_send",
        nx_kern_return_str(kr_cargo_type_after), kr_cargo_type_after,
        false, 0);
    nx_result_emit_return(&j, "mach_msg_receive_after_invalid_send",
        nx_msg_return_str(mr_receive), mr_receive, false, 0);
    if (mr_receive == MACH_MSG_SUCCESS) {
        nx_result_emit_return(&j, "mach_port_deallocate_delivered_unexpected",
            nx_kern_return_str(kr_deallocate_delivered),
            kr_deallocate_delivered, false, 0);
    }
    ...
    nx_result_emit_right_delta(&j,
        "invalid descriptor disposition service header",
        "service_port",
        "MACH_PORT_RIGHT_SEND",
        kr_service_refs_before == KERN_SUCCESS ? service_refs_before : -1,
        kr_service_refs_after == KERN_SUCCESS ? service_refs_after : -1,
        -1, -1, "unchanged");
    nx_result_emit_right_delta(&j,
        "invalid descriptor disposition cargo descriptor",
        "cargo_port",
        "MACH_PORT_RIGHT_SEND",
        kr_cargo_refs_before == KERN_SUCCESS ? cargo_refs_before : -1,
        kr_cargo_refs_after == KERN_SUCCESS ? cargo_refs_after : -1,
        -1, -1, "unchanged");
    nx_json_end_array(&j);

    nx_json_key(&j, "observations");
    nx_json_begin_object(&j);
    nx_json_key_string(&j, "invalid_descriptor_disposition",
        hex32(OB24_INVALID_DISPOSITION));
    nx_json_key_string(&j, "invalid_descriptor_disposition_label",
        disposition_str(OB24_INVALID_DISPOSITION));
    nx_json_key_string(&j, "descriptor_type",
        descriptor_type_str(MACH_MSG_PORT_DESCRIPTOR));
    nx_json_key_bool(&j, "send_rejected", send_rejected);
    nx_json_key_string(&j, "send_return", nx_msg_return_str(mr_send));
    nx_json_key_bool(&j, "message_delivered", mr_receive == MACH_MSG_SUCCESS);
    ...
    nx_json_key_string(&j, "received_descriptor_type",
        descriptor_type_str(received_descriptor_type));
    nx_json_key_string(&j, "received_descriptor_disposition",
        disposition_str(received_descriptor_disposition));
    nx_json_key_bool(&j, "service_send_refs_unchanged",
        service_refs_unchanged);
    nx_json_key_bool(&j, "cargo_send_refs_unchanged", cargo_refs_unchanged);
    nx_json_key_bool(&j, "cargo_type_unchanged", cargo_type_unchanged);
    nx_json_key_string(&j, "cargo_type_before",
        nx_port_type_str(cargo_type_before));
    nx_json_key_string(&j, "cargo_type_after",
        nx_port_type_str(cargo_type_after));
    nx_json_key_int(&j, "service_refs_before", service_refs_before);
    nx_json_key_int(&j, "service_refs_after", service_refs_after);
    nx_json_key_int(&j, "cargo_refs_before", cargo_refs_before);
    nx_json_key_int(&j, "cargo_refs_after", cargo_refs_after);

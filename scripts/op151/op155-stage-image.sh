#!/bin/sh
# op155-stage-image.sh — THIN GLUE (per op-147m). Stages freeze-N for op-155.
# Same as op151-stage-image.sh but uses op155-rc.local.template (HB→console).
set -eu
...
rclocal_template="$repo_root/scripts/op151/op155-rc.local.template"
...
echo "[op155-stage] cp golden → $out_image"
...
printf 'op155_stage_status=0 image=%s soak_duration=%s\n' "$out_image" "$soak_duration"
